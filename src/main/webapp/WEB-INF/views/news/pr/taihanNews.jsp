...
<section id="content" class="sub-wrap pr">
    ...
    <div id="container" class="content-wrap">
        ...
        <div class="title-wrap">
            ...
            <p><spring:message code="taihanNews.desc" text="대한전선의 새로운 소식과 관련 정보를 전해 드립니다."></spring:message></p>
        </div>

        <div class="content-box bg">
			<div class="inner-box">
				<div class="title-wrap2">
					<h4 class="title-type">Highlights</h4>
				</div>

				<div class="news-swiper">
					<div class="swiper-wrapper">
						<c:forEach var="item" items="${topFixList}" varStatus="status">
							<div class="swiper-slide">
								<a href="javascript:void(0);" onclick="goDetail('${item.idx}')" class="news-box">
									<div class="img-flex"><img src="${CON_UPLOAD_URL}${item.sysThumImgFile}" onerror="this.src='${CON_STATIC_URL}/asset/images/list.jpg';" alt="${item.thumImgText}"></div>
									<div class="text-box">
										<span class="date">${item.regDt}</span>
										<strong class="title">${item.title}</strong>
										<p class="desc">
											${cmnUtils:unescapeXmlWithOutReplaceBR(item.content)} 
										</p>
									</div>
								</a>
							</div>
			...
			<div class="inner-box">
				<div class="board-wrap">
					...
					<div class="board-top">
						...
						<input type="hidden" id="pageNo" name="pageNo" value="${taihanNews.pageNo}">
						<input type="hidden" id="rowSize" name="rowSize" value="${taihanNews.rowSize}">
						<!-- search -->
						<div class="search-box v2">
							<div class="select-type">
								<select id="schField" name="schField" >	
									<option value="TITLE" <c:if test="${taihanNews.schField eq 'TITLE'}">selected</c:if>>
									  <spring:message code="taihanNews.schText1" text="제목" /></option>
									<option value="CONTENT" <c:if test="${taihanNews.schField eq 'CONTENT'}">selected</c:if>>
									  <spring:message code="taihanNews.schText2" text="내용" /></option>
								</select>
							</div>
							<div class="search-form">
								<input type="text" id="listSchKey" name="schKey" class="input-type" value="${taihanNews.schKey}" 
					...
					<c:if test="${empty list}">
						<div class="nodata-box v2">
							...
							<p>
								<c:if test="${not empty taihanNews.schKey}">
									<strong>"${taihanNews.schKey}"검색결과가 없습니다.</strong>
								</c:if>
								<c:if test="${empty taihanNews.schKey}">
							...
							<p>
								<c:if test="${not empty taihanNews.schKey}">
									<strong>0 results for "${taihanNews.schKey}"</strong>
								</c:if>
								<c:if test="${empty taihanNews.schKey}">
